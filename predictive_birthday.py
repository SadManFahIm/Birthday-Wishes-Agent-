...
━━━━━━━━━━━━━━━━━━━━━━
Birthday unknown হলে — LinkedIn posts, profile info, এবং past activity দেখে
approximate birthday guess করে। Result SQLite-এ save হয় এবং scheduled
wishing-এ use হয়।
...
def init_predicted_birthday_table():
    with sqlite3.connect(DB_FILE) as conn:
        ...
        """)
        conn.commit()
    logger.info("🗄️  predicted_birthdays table ready.")
...
def get_todays_predicted_birthdays() -> list[dict]:
    """আজকের predicted birthday যাদের wish এখনো যায়নি।"""
    today = date.today()
...
async def predict_birthday_for_contact(
    ...
    """
    One contact-এর জন্য full prediction pipeline চালায়:
...
async def run_predictive_birthday(
    ...
    """
    Batch prediction — contacts যাদের birthday LinkedIn-এ নেই তাদের
    posts/profile দেখে birthday guess করে।
    ...
    for c in contacts[:max_predictions]:
        ...
        # Cached check — আজকে already predicted?
        ...
        # আজকে birthday match করে কিনা check করো
        today = date.today()
        is_birthday_today = (
        ...
        # আজকে birthday হলে এবং confidence threshold পার করলে wish পাঠাও
...
async def send_todays_predicted_wishes(
    ...
    """
    DB থেকে আজকের predicted birthdays বের করে wish পাঠায়।
    daily_job()-এ call করো।
    """
